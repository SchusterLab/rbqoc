...
@inline astate_sample_inds(sample_state_index::Int, sample_index::Int) = (
    (
        ASTATE_SIZE_BASE + (sample_state_index - 1) * SAMPLES_PER_STATE * HDIM_ISO + (sample_index - 1) * HDIM_ISO + 1
    ):(
        ASTATE_SIZE_BASE + (sample_state_index - 1) * SAMPLES_PER_STATE * HDIM_ISO + sample_index * HDIM_ISO
    )
)
...
function RD.discrete_dynamics(::Type{RK3}, model::Model, astate::Array{T, 1},
    ...
    # base dynamics
    astate_ = zeros(ASTATE_SIZE)
    negi_hc = astate[CONTROLS_IDX[1]] * NEGI_H1_ISO
    h_prop = exp(dt * (FQ_NEGI_H0_ISO + negi_hc))
    astate_[STATE1_IDX] = h_prop * astate[STATE1_IDX]
    astate_[STATE2_IDX] = h_prop * astate[STATE2_IDX]
    astate_[INTCONTROLS_IDX] = astate[INTCONTROLS_IDX[1]] + dt * astate[CONTROLS_IDX[1]]
    astate_[CONTROLS_IDX] = astate[CONTROLS_IDX[1]] + dt * astate[DCONTROLS_IDX[1]]
    astate_[DCONTROLS_IDX] = astate[DCONTROLS_IDX[1]] + dt * acontrol[D2CONTROLS_IDX[1]]
    

    # # unscented transform
    # for i = 1:SAMPLE_STATE_COUNT
    #     unscented_transform!(astate, astate_, negi_hc, i)
    # end
...
function Cost(Q::Diagonal{T,Array{T,1}}, R::Diagonal{T,Array{T,1}},
    ...
    for i = 1:SAMPLE_STATE_COUNT
        ...
        for j = 1:SAMPLES_PER_STATE
            ...
            println("targeto: $(targeto), inds[$(i), $(j)]: $(inds)")
            # For reasons unkown to the author, throwing a -1 in front
            # of the gate error Hessian makes the cost function work.
            foo = -1 * q_sample_states[i]
            bar = hessian_gate_error_iso2(target_states; s2o=targeto)
            hess_astate[inds, inds] =  foo * bar 
        end
    end
    hess_astate += Q
...
function TO.stage_cost(cost::Cost{N,M,T}, astate::Array{T,1}) where {N,M,T}
    ...
    for i = 1:SAMPLE_STATE_COUNT
        targeto = (i - 1) * SAMPLE_STATE_COUNT * HDIM_ISO
        for j = 1:SAMPLES_PER_STATE
            astateo = astate_sample_inds(i, j)[1] - 1
            cost.q_sample_states[i] * gate_error_iso2(astate, target_states, astateo, targeto)
        end
    end
    return cost_
end
...
function TO.gradient!(E::TO.QuadraticCostFunction, cost::Cost{N,M,T}, astate::Array{T,1}) where {N,M,T}
    ...
    for i = 1:SAMPLE_STATE_COUNT
        ...
        for j = 1:SAMPLES_PER_STATE
            inds = astate_sample_inds(i, j)
            E.q[inds] = (cost.q_sample_states[i]
                         * jacobian_gate_error_iso2(astate, cost.target_states, inds[1] - 1, targeto))
        end
    end
    return false
end
