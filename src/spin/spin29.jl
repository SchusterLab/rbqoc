"""
spin29.jl - unscented transform robustness for the δfq problem, this is the old working
spin23.jl
"""
...
const ASTATE_SIZE = ASTATE_SIZE_BASE + SAMPLE_STATE_COUNT * SAMPLES_PER_STATE * HDIM_ISO
...
@inline Base.copy(cost::Cost{N,M,T}) where {N,M,T} = Cost{N,M,T}(
    cost.Q, cost.R, cost.q, cost.c,
    cost.target_state1, cost.q_ss, cost.active_samples
)
...
function TO.stage_cost(cost::Cost{N,M,T}, astate::SVector{N}) where {N,M,T}
    ...
    for i = 1:SAMPLE_STATE_COUNT
        for j in cost.active_samples
            ...
            cost_ = cost_ + gate_error_iso2(astate, cost.target_state1; s1o=sample_idx[1] - 1)
        end
    end
    return cost_
end
...
function TO.hessian!(E::TO.QuadraticCostFunction, cost::Cost{N,M,T},
    ...
    for i = 1:SAMPLE_STATE_COUNT
        for j in cost.active_samples
            sample_idx = astate_sample_inds(i, j)
            E.Q[sample_idx, sample_idx] = hess_sample
        end
    end
    E.Q = E.Q + cost.Q
    return true
...
function run_traj(;gate_type=xpiby2, evolution_time=60., solver_type=altro,
    ...
    x0 = zeros(n)
    xf = zeros(n)
    target_states = Array{SVector{HDIM_ISO},1}(undef, SAMPLE_STATE_COUNT)
    ...
    Q = Diagonal(SVector{n}([
        ...
        fill(0, SAMPLES_PER_STATE * HDIM_ISO);
    ]))
    Qf = Q * N
    ...
    cost_k = Cost(Q, R, xf, target_states[1], qs[5], active_samples)
    cost_f = Cost(Qf, R, xf, target_states[1], N * qs[5], active_samples)
