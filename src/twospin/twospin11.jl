...
function RD.discrete_dynamics(::Type{RK3}, model::Model, astate::SVector,
    ...
    h_prop = exp(dt * (NEGI_H0_TWOSPIN_ISO
                       + astate[CONTROLS_IDX[1]] * NEGI_H1_TWOSPIN_ISO_1
                       + astate[CONTROLS_IDX[2]] * NEGI_H1_TWOSPIN_ISO_2
                       + astate[CONTROLS_IDX[3]] * NEGI_H1_TWOSPIN_ISO_3))
...
function run_traj(;gate_type=zzpiby2, evolution_time=70., solver_type=altro,
                  ...
                  max_cost_value=1e8, qs=[1e0, 1e0, 1e0, 1e0, 1e0, 1e-1, 1e-1],
                  benchmark=false,)
    # model configuration
    model = Model()
    n = state_dim(model)
