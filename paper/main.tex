...
their analytic counterparts, a similar scaling for their single gate error improvements
$\sim 3 \cdot 10^{-5} \rightarrow \sim 7 \cdot 10^{-6}$.
...
We consider two numerical strategies for achieving robustness
...
${\lvert \braket{\partial^{l}_{\lambda} \psi_{k} | \partial^{l}_{\lambda}
    \psi_{k}} \rvert}^{2}$, $l \in \{1, \dots, m\}$.
...
\end{align}
Matrix exponential schemes that account for the non-linear
...
The differences between the final states and the target
state are penalized, resulting in quadratic costs at
each knot point ${\lvert \braket{\tilde{\psi}^{\pm}_{k} | \tilde{\psi}^{\pm}_{k}}
  \rvert}^{2}, \ket{\tilde{\psi}^{\pm}_{k}} = \ket{\psi^{\pm}_{k}} - \ket{\psi_{f}}$.
We sample two copies ($\ket{\psi^{\pm}}$) of four initial states with this method,
...
trajectories are insensitive to some parameter deviations and therefore
they are undesirable samples for those operations.
...
resulting in $O(n^2)$ additional states in the augmented state vector.
...
qubit frequency detuning, it achieves a gate error of $\sim 4.5 \cdot 10^{-5}$, sufficient
...
a function of the gate duration which approaches zero at four multiples
of $t_{Z/2}$. We observe that on a range of problems at small parameter detunings,
