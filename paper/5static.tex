...
isomorphism \eqref{eq:isomorphism}.
...
to the uncertain parameter, which is encoded in the $m$\textsuperscript{th}-order
state derivative $\ket*{\partial_{f_{q}}^{m} \psi} \equiv \partial_{f_{q}}^{m} \ket*{\psi}$.
...
of the final augmented state to zero. Intuitively, this corresponds to penalizing
...
in the detuning.
At a one-percent detuning ($\lvert \delta f_{q} \rvert / f_{q} = 1\%$),
