...
\begin{figure*}[ht]
  ...
  \caption{
    ...
    absolute flux and $T_{1}$ time for each pulse.
...
and its norm is penalized in the objective by setting
the corresponding element of the target augmented state to zero.
$T_{1}$ as a function of the flux is obtained by evaluating
a spline fit to experimental data, see Figure \ref{fig:longitudeb}.
\textcolor{blue}{[Phys. Rev. will want this to read 1(b).
    Not sure how to change this the way you are doing automatic referencing.]}
...
\textcolor{blue}{[abrupt start of a new paragraph; provide
    transition/motivation for what's next; the entire following
    paragraph feels a bit out of place/inserted after the fact.]}
\textcolor{red}{[better?]}
...
\textcolor{blue}{[improvements are significant FOR THE CONSTRAINTS?]}
\textcolor{red}{[changed "for" to "given"]}
